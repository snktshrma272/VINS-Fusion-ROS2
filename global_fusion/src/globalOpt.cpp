...
void GlobalOptimization::inputVPS(double t, double x, double y, double z, double posAccuracy)
{
	// double xyz[3];
	// GPS2XYZ(latitude, longitude, altitude, xyz);
	// vector<double> tmp{xyz[0], xyz[1], xyz[2], posAccuracy};
    // //printf("new vps: t: %f x: %f y: %f z:%f \n", t, x, y, z);
	VPSPositionMap[t] = vector<double> {x,y,z,posAccuracy};
    newVPS = true;
...
void GlobalOptimization::optimize()
{
    while(true)
    {
        ...
        {
            newVPS = false;
            printf("global optimization vps\n");
            TicToc globalOptimizationTime;
            ...
            map<double, vector<double>>::iterator iterVIO, iterVIONext, iterVPS;
            int i = 0;
            for (iterVIO = localPoseMap.begin(); iterVIO != localPoseMap.end(); iterVIO++, i++)
            {
                //vio factor
                ...
                //vps factor
                double t = iterVIO->first;
                iterVPS = VPSPositionMap.find(t);
                if (iterVPS != VPSPositionMap.end())
                {
                    ceres::CostFunction* vps_function = TError::Create(iterVPS->second[0], iterVPS->second[1], 
                                                                       iterVPS->second[2], iterVPS->second[3]);
                    //printf("inverse weight %f \n", iterGPS->second[3]);
                    problem.AddResidualBlock(vps_function, loss_function, t_array[i]);
                    ...
                    double **jaco = new double *[1];
                    jaco[0] = new double[3 * 3];
                    vps_function->Evaluate(para, tmp_r, jaco);
            ...
            for (int i = 0; i < length; i++, iter++)
            {
            	...
            	{
            	    Eigen::Matrix4d WVIO_T_body = Eigen::Matrix4d::Identity(); 
            	    Eigen::Matrix4d WVPS_T_body = Eigen::Matrix4d::Identity();
            	    double t = iter->first;
            	    WVIO_T_body.block<3, 3>(0, 0) = Eigen::Quaterniond(localPoseMap[t][3], localPoseMap[t][4], 
            	                                                       localPoseMap[t][5], localPoseMap[t][6]).toRotationMatrix();
            	    WVIO_T_body.block<3, 1>(0, 3) = Eigen::Vector3d(localPoseMap[t][0], localPoseMap[t][1], localPoseMap[t][2]);
            	    WVPS_T_body.block<3, 3>(0, 0) = Eigen::Quaterniond(globalPose[3], globalPose[4], 
            	                                                        globalPose[5], globalPose[6]).toRotationMatrix();
            	    WVPS_T_body.block<3, 1>(0, 3) = Eigen::Vector3d(globalPose[0], globalPose[1], globalPose[2]);
            	    WGPS_T_WVIO = WVPS_T_body * WVIO_T_body.inverse();
            	}
            }
            updateGlobalPath();
            //printf("global time %f \n", globalOptimizationTime.toc());
